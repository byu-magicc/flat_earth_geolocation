...
Geolocator::Geolocator(Ptr<CameraModel> camModel)
{
    // store the camera model for use in geolocating
    _cameraModel = camModel;
}

// ----------------------------------------------------------------------------

void Geolocator::transform(vector<Point2f>& measurements,
...
cv::Mat Geolocator::rot_v2b(double phi, double theta, double psi)
{
    // Euler Rotation from vehicle to vehicle-1 frame
    Mat rot_v2v1 = (Mat_<float>(3, 3) <<   cos(psi), sin(psi),      0   ,
                                          -sin(psi), cos(psi),      0   ,
                                               0   ,     0   ,      1     );

    // Euler Rotation from vehicle-1 to vehicle-2 frame
    Mat rot_v12v2 =(Mat_<float>(3, 3) << cos(theta),   0   , -sin(theta),
                                              0    ,   1   ,       0    ,
                                         sin(theta),   0   ,  cos(theta)  );

    // Euler Rotation from vehicle-2 to body frame
    Mat rot_v22b = (Mat_<float>(3, 3) <<       1   ,     0    ,      0   ,
                                               0   ,  cos(phi),  sin(phi),
                                               0   , -sin(phi),  cos(phi)  );

    // Euler Rotation from vehicle frame to body frame
    return rot_v22b * rot_v12v2 * rot_v2v1;
}

// ----------------------------------------------------------------------------

cv::Mat Geolocator::rot_b2g(double r, double p, double y)
{
    // Euler Rotation from gimbal to gimbal-1 frame
    Mat rot_g2g1 = (Mat_<float>(3, 3) <<  cos(y), sin(y),      0,
                                         -sin(y), cos(y),      0,
                                              0 ,     0 ,      1  );

    // Euler Rotation from gimbal-1 to gimbal-2 frame
    Mat rot_g12g2 = (Mat_<float>(3, 3) <<       1,     0  ,      0 ,
                                               0,  cos(r),  sin(r),
                                               0, -sin(r),  cos(r)  );

    // Euler Rotation from gimbal-2 to body frame
    Mat rot_g22b =(Mat_<float>(3, 3) <<  cos(p),   0, -sin(p),
                                               0,   1,       0,
                                          sin(p),   0,  cos(p)  );

    // Euler Rotation from gimbal frame to body frame
    // Note: fcu_sim gimbal plugin sequence is yaw, roll, pitch
    // TODO: Figure out a way to abstract this? Or all are gimbals this way?
    return rot_g22b * rot_g12g2 * rot_g2g1;
}

// ----------------------------------------------------------------------------

cv::Mat Geolocator::rot_g2c()
{
    // Euler Rotation from the gimbal from to camera frame
    return (Mat_<float>(3, 3) << 0,1,0, 0,0,1, 1,0,0);
}

cv::Mat Geolocator::d_b2g()
{
    // translation from body to gimbal
    // TODO: abstract out and make a parameter
    return (Mat_<float>(3, 1) << 0.2, 0, 0.1);
}
